...
def get_ai_answer(user_msg, is_premium=False):
    ...
    if is_premium:
        max_tokens = 800 # Premium ko detail
        prompt = f"{system}\nUser: {user_msg}\nGive a detailed, step-by-step answer."
    else:
        max_tokens = 250 # Free wale ko short
        prompt = f"{system}\nUser: {user_msg}\nKeep answer short and to the point."

    response = model_flash.generate_content(prompt)
    return response.text.strip()

def get_yt_analysis(user_input, is_premium=False):
...
async def yt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    analysis = get_yt_analysis(user_input, is_premium)
...
async def chat_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply = get_ai_answer(update.message.text, is_premium)
